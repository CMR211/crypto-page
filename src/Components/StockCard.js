...
export default function StockCard( {stock} ) {
  ...
  console.log(stock)
  const { name, price, changesPercentage, symbol } = stock

  // round daily change to #.## format
  const changeRounded = Math.round(changesPercentage * 100) / 100
  ...
  return (
    <a className='popular-card__link' href={gotoLink()}>
      <div className={`popular-card popular-card-${symbol}`} onClick={event => window.location.href = gotoLink()}>

        <img className='popular-card__icon' src={`/assets/${symbol}.png`} alt={`${symbol}-icon`} />

        <p className='popular-card__title--stock'>{symbol}<br /><span className='popular-card__title--stockname'>{name}</span></p>
        ...
        <p className={'popular-card__change ' + getChangeColor()}>{`(${changeRounded}%)`}</p>
